...
const ProductCardVR = ({ product }) => {
    ...
    return (
        <div className='w-full relative h-auto rounded-2xl shadow-[0_2px_10px_rgba(0,0,0,0.2)] p-2 flex gap-2'>
            ...
            <Image src={product.imagesLink?.[0]} width={100} height={100} alt={product.title || 'Product Image'} className="w-35 h-35 min-[430px]:w-45 min-[430px]:h-45 sm:w-35 sm:h-35 md:w-55 md:h-55 object-cover  bg-white rounded-2xl"></Image>
            <div className="flex flex-col w-full gap-2 justify-between items-start p-2">
                <div>
                    ...
                    <div className="flex gap-1">
                        {hasDiscount && <p className="text-text-muted text-md min-[430px]:text-xl sm:text-sm md:text-2xl font-semibold line-through">{beforePrice}৳</p>}
                        <p className="text-md min-[430px]:text-2xl text-red-500 sm:text-lg md:text-3xl font-bold">{price}৳</p>
                    </div>
                </div>
